...
def evaluate_accuracy(result_path, subjects, sessions, compression_method, fine_tuned=False, ondevice=False):
    ...
    for subject in subjects:
        for session in sessions:
            if fine_tuned:
                ...
                datapath = "%s/emager_%s_%s_%s_evaluation_finetuned.npz"%(result_path, subject, session, compression_method)
            elif ondevice:
                datapath = "%s/emager_%s_%s_%s_evaluation_ondevice.npz"%(result_path, subject, session, compression_method)
            else:    
                datapath = "%s/emager_%s_%s_%s_evaluation.npz"%(result_path, subject, session, compression_method)
    ...
    print("Accuracy")
    print("{:.2f}".format(100*np.mean(global_accuracy)), "+-", "{:.2f}".format(100*np.std(global_accuracy)))

    print("Accuracy majority vote")
    print("{:.2f}".format(100*np.mean(global_accuracy_maj)),"+-", "{:.2f}".format(100*np.std(global_accuracy_maj),"\n"))
...
def evaluate_time(result_path, subjects, sessions, compression_methods, fine_tuned=False, ondevice=False):
...
if __name__ == "__main__":
    ...
    evaluate_accuracy(result_path, subjects, sessions, "root", fine_tuned=True, ondevice=False)
    evaluate_time(result_path, subjects, sessions, compression_methods, fine_tuned=False, ondevice=True)
