...
def create_average_model(nb_class, shape):
    activation = 'relu'
    hidden_layer_1 = 64
    hidden_layer_2 = 64
    model = tf.keras.Sequential([
        tf.keras.layers.Conv2D(filters = hidden_layer_1, kernel_size = 3, strides = 1, padding = 'same', activation=activation, input_shape=shape),
...
def coralemg_net(nb_class, shape):
    activation = 'relu6'
    hidden_layer = 64
    model = tf.keras.Sequential([
        tf.keras.layers.Conv2D(filters = hidden_layer, kernel_size = 3, strides = 1, padding = 'same', activation=activation, input_shape=shape),
        tf.keras.layers.BatchNormalization(),
