...
def naive_bitshiftcd(data, bits):
...
def log_c(data, atg):
    ...
    Could optimize to only have 1 log since log_a(x) = log_b(x)/log_b(a)

    ATG = Andre The Giant value of average EMG activation

    Returns the log of data scaled to [0,255]
    """
    log_data = 255*np.emath.logn(atg, data)
    return np.clip(np.round(log_data), 0, 255).astype(np.uint8)
...
def nroot_c(data, exp, max):
    ...
    return np.round(rt_data).astype(np.uint8)
